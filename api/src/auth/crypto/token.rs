...
mod tests {
    ...
    #[test]
    fn invalid_key() {
        assert!(TokenEncryption::new("invalid").is_err());
        assert!(TokenEncryption::new("").is_err());
        assert!(TokenEncryption::new("short").is_err());
    }
    ...
    fn different_keys_cant_decode() {
        ...
        let crypto2 = TokenEncryption::new(key2).unwrap();
